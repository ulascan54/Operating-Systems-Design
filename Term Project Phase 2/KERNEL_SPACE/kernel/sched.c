...
// Variable declarations for calculating fair-share cpu times
float weight;           // 1 / p_of_users[uid-1000]
int process_count;        // Process count for users
...
repeat_schedule:
	...
	if (scheduler_flag == 1) // Fair Share scheduler
	{
		if (unlikely(!c))
		{
            ...
            for_each_task(p){
                ...
                } else { // Root and other user's that we don't care :)
